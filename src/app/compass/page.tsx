...
export default function CompassPage() {
  ...
  // Check if this is an upgraded reflection
  const isUpgradedReflection = !!currentReflection?.upgradedFrom;
          ...
          {showSectionIntro && !showCompletion ? (
            ...
            >
              ...
              <motion.p
                ...
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.2 }}
                className="text-[var(--text-muted)] text-center mb-4"
              >
                {currentQuestion.prompt}
              </motion.p>

              {/* Pre-fill indicator for upgraded reflections */}
              {isUpgradedReflection && currentValue && (
                typeof currentValue === 'string' ? currentValue.trim().length > 0 :
                Array.isArray(currentValue) ? currentValue.some(v => v?.trim?.().length > 0) :
                ...
                >
                  <p className="text-sm text-green-300 flex items-center justify-center gap-2">
                    ...
                    Pre-filled from your previous reflection — feel free to edit
                  </p>
                </motion.div>
              )}
